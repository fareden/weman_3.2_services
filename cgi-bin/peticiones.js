	...
	function procesaGet(peticion) {
		...
		if (texto.query.saludar === undefined) { // con los puntitos concatenas   variable.lugar.tipo?
			var textInvertir = texto.query.texto;
			return voltearPalabra(textInvertir);
		} else{
			var textSaludar = url.parse(peticion.url, true); //URL parse returns  a URL object
			var textSaludarDos = textSaludar.query.texto;
			console.log (textSaludarDos)
			return mostrarSaludo(textSaludarDos) + textSaludarDos;
		}
	};
